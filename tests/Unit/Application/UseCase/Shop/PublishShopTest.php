<?php
/**
 * PublishShop UseCase 單元測試
 *
 * 對應規格：specs/2026-05-06-profit-shop-design.md §4.1
 *
 * @group profit_shop
 * @group application
 * @group usecase
 */

declare( strict_types=1 );

namespace Tests\Unit\Application\UseCase\Shop;

use J7\PowerShop\Domains\ProfitShop\Application\UseCase\Shop\PublishShop;
use J7\PowerShop\Domains\ProfitShop\Domain\Entity\ProfitShop;
use J7\PowerShop\Domains\ProfitShop\Domain\Exception\ProfitShopNotFound;
use J7\PowerShop\Domains\ProfitShop\Domain\ValueObject\ProfitRate;
use J7\PowerShop\Domains\ProfitShop\Domain\ValueObject\ShopMode;
use PHPUnit\Framework\TestCase;
use Tests\Unit\Application\Fakes\InMemoryProfitShopRepository;

/**
 * PublishShop UseCase 測試
 */
final class PublishShopTest extends TestCase {

	private InMemoryProfitShopRepository $shopRepo;

	protected function setUp(): void {
		parent::setUp();
		$this->shopRepo = new InMemoryProfitShopRepository();
	}

	/**
	 * happy：draft 賣場 → 切換為 publish
	 *
	 * @group happy
	 */
	public function test_publishes_draft_shop(): void {
		$this->shopRepo->seed( $this->build_shop( 400, 'draft' ) );

		$useCase = new PublishShop( shopRepo: $this->shopRepo );

		$output = $useCase->execute( id: 400 );

		$this->assertSame( 'publish', $output->status );
		$shop = $this->shopRepo->find( 400 );
		$this->assertNotNull( $shop );
		$this->assertSame( 'publish', $shop->status() );
	}

	/**
	 * happy：已是 publish → 操作幂等成功（不該拋例外）
	 *
	 * @group happy
	 */
	public function test_publishing_already_published_shop_is_idempotent(): void {
		$this->shopRepo->seed( $this->build_shop( 401, 'publish' ) );

		$useCase = new PublishShop( shopRepo: $this->shopRepo );

		$output = $useCase->execute( id: 401 );

		$this->assertSame( 'publish', $output->status );
	}

	/**
	 * error：賣場不存在 → 拋出 ProfitShopNotFound
	 *
	 * @group error
	 */
	public function test_throws_not_found_when_shop_does_not_exist(): void {
		$useCase = new PublishShop( shopRepo: $this->shopRepo );

		$this->expectException( ProfitShopNotFound::class );
		$useCase->execute( id: 9999 );
	}

	private function build_shop( int $id, string $status ): ProfitShop {
		return new ProfitShop(
			id: $id,
			title: '草稿賣場',
			slug: 'draft-shop',
			status: $status,
			mode: ShopMode::PAGE,
			partner_term_id: 5,
			rate: new ProfitRate( 10 ),
			items: [],
			settings: []
		);
	}
}
